...
Make sure that you are still in the BigQuery Console for this step. In the Explorer tab, click the + Add data button then select Public datasets.
...
Note: If you are unable to see the ncaa_basketball then click on + Add data > Star a project by name. Enter bigquery-public-data as the project name and then click STAR.
...
In SQL query editor, which is located above the table details section, copy and paste the following into that field:
SELECT
  season,
...
In the SQL query editor, copy and paste the following query and then click Run:
...
Now run the following command in the SQL query editor
...
You can view how many training runs the model takes by clicking the Training tab and selecting the Table option.
